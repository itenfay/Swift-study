...
//IntStackA 结构体实现了 Container 协议的三个要求，其原有功能也不会和这些要求相冲突。

//此外，IntStackA 在实现 Container 的要求时，指定 ItemType 为 Int 类型，即 typealias ItemType = Int，从而将 Container 协议中抽象的 ItemType 类型转换为具体的 Int 类型。
...
struct StackA<Element>: Container {
...
//为了遵守了 Container 协议，ItemType 类型也必须遵守 Equatable 协议。
...
//C1 必须符合 Container 协议（写作 C1: ContainerB）。
//C2 必须符合 Container 协议（写作 C2: ContainerB）。
...
//上述示例中的泛型 where 子句要求 Item 符合协议，但也可以编写一个泛型 where 子句去要求 ItemType 为特定类型。例如：
...
class GenericityUsage: NSObject {
    ...
    func execute() {
        ...
        // 栈中现在有 4 个字符串
        print("stackOfStrings.items are \(stackOfStrings.items)")
        //移除并返回栈顶部的值 "cuatro"，即将其出栈：
        let fromTheTop = stackOfStrings.pop()
        print("popItem is \(fromTheTop), stackOfStrings.items are \(stackOfStrings.items)")
        ...
        print("stringIndex is \(String(describing: stringIndex))")

        //下面演示了 allItemsMatch(_:_:) 函数的使用：
        let arrayOfStrings = ["uno", "dos", "tres"]
        
        if allItemsMatch(stackOfStrings.items, arrayOfStrings) {
        ...
        //编写一个泛型 where 子句去要求 ItemType 为特定类型
