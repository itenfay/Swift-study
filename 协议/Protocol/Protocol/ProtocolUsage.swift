...
//  Protocol
//
//  Created by Dyfei on 2017/10/23.
...
protocol SomeProtocol {
    ...
    var doesNotNeedToSettable: Int { get }
}

//在协议中定义类型属性时，总是使用 static 关键字作为前缀。当类类型遵循协议时，除了 static 关键字，还可以使用 class 关键字来声明类型属性：
protocol AnotherProtocolB {
...
class SomeClassB: SomeProtocolC {
...
protocol SomeProtocolC2 {
    init()
}

class SomeSuperClassC {
    init() {
        // 这里是构造器的实现部分
    }
}

class SomeSubClass: SomeSuperClassC, SomeProtocolC2 {
...
extension SnakesAndLadders: PrettyTextRepresentable {
    var prettyTextualDescription: String {
        var output: String = textualDescription + ":\n"
...
class ProtocolUsage: NSObject {
    ...
    func execute() {
        ...
        let LCGenerator = LinearCongruentialGenerator()
        print("Here's a random number: \(LCGenerator.random())")
        // 打印 “Here's a random number: 0.37464991998171”
        print("And another one: \(LCGenerator.random())")
        // 打印 “And another one: 0.729023776863283”
        
        var lightSwitch = OnOffSwitch.Off
        print("Now lightSwitch value is \(lightSwitch.toggle())")
        ...
        let seattle = City(name: "Seattle", latitude: 47.6, longitude: -1242.3)
